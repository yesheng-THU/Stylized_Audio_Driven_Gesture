...
class Locomotion():
    ...
    def __init__(self, hparams, is_training, log_path):
        ...
        if is_training:
            ...
            validation_text = train_text[-1000:, :]
        ...
        if is_training:
            self.train_dataset = MotionDataset(train_data[:, :, :audio_start], train_data[:, :, audio_start:], train_text, train_style,
                                           hparams.Data.seqlen,
                                           hparams.Data.n_lookahead, hparams.Data.dropout)
            self.validation_dataset = MotionDataset(validation_data[:, :, :audio_start], validation_data[:, :, audio_start:], validation_text, validation_style,
                                                hparams.Data.seqlen, hparams.Data.n_lookahead, hparams.Data.dropout)
        
        self.test_dataset = TestDataset(all_test_data[:, :, :audio_start], all_test_data[:, :, audio_start:], test_text, test_style)
        self.FGD_val_dataset = TestDataset(all_FGD_data[:, :, :audio_start], all_FGD_data[:, :, audio_start:], FGD_val_text, FGD_val_style)
