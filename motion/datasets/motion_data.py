...
class MotionDataset(Dataset):
    ...
    def __init__(self, control_data, joint_data, text_data, style, seqlen, n_lookahead, dropout):
        """
        Args:
        ...
        print("self.text:" + str(self.text.shape))
    ...
    def __getitem__(self, idx):
        ...
        if self.dropout > 0.:
            ...
            sample = {'x': self.x[idx, :, :], 'cond': cond_masked, 'text': self.text[idx, :], 
                    'seed': self.seed[idx, :, :], 'control': self.control[idx, :, :], 'style': self.style[idx]}
        else:
            sample = {'x': self.x[idx, :, :], 'cond': self.cond[idx, :, :], 'text': self.text[idx, :],
                    'seed': self.seed[idx, :, :], 'control': self.control[idx, :, :], 'style': self.style[idx]}
...
class TestDataset(Dataset):
    ...
    def __init__(self, control_data, joint_data, text_data, style):
        """
        Args:
        control_data: The control input
        joint_data: body pose input
        ...
        print('test_dataset', self.text.shape)

    def __len__(self):
        return self.autoreg.shape[0]

    def __getitem__(self, idx):
        sample = {'autoreg': self.autoreg[idx, :], 'control': self.control[idx, :], 'text': self.text[idx, :], 'style': self.style[idx]}
        return sample
