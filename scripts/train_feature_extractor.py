...
def create_video_and_save(save_path, target, output, mean_data, title,
    ...
    try:
        video_path = '{}/{}_{}.mp4'.format(save_path, title, epoch)
...
if __name__ == '__main__':
    ...
    log_dir = os.path.join(hparams_motion.Dir.log_root, "trinity_feature")
    print("log_dir:" + str(log_dir))
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    data_motion = dataset_motion(hparams_motion, True)
...
model = Trinity_Autoencoder(n_pose, pose_dim).to(device)
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        torch.save(model, os.path.join(log_dir, f'vae_{epoch+1}.pth'))
        with torch.no_grad():
            # Save the reconstruct motion sequence
            model.eval()
            out = x_recon[0:1,:,:]
            out = out.cpu().numpy()
            data_motion.save_val(out, os.path.join(log_dir, f'fake_{epoch}'))
