...
def generate_sample(graph, batch, hparams, eps_std=1.0):
        ...
        with torch.no_grad():
            for i in range(0, control_all.shape[1] - seqlen - n_lookahead, clip_length):
                ...
                seed = torch.from_numpy(sampled_all[:, :seqlen, :]).to(device)
                seed = seed.permute(0, 2, 1).float()
                control = torch.from_numpy(control_all).to(device)
                control = control.permute(0, 2, 1).float()

                # sample from Moglow
                sampled = graph(z=None, cond=cond, eps_std=eps_std, reverse=True, seed=seed, control=control, text=None, z_shape=[nn, 27, 1])
...
def evaluate_testset(data_motion, test_data_loader, generator, embed_space_evaluator, hparams, step=0):
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            # create_video_and_save(".", target_origin.cpu().numpy().reshape(-1, 27), out_dir_vec.cpu().numpy().reshape(-1, 27), mean_data, "test",
    ...
    if embed_space_evaluator and embed_space_evaluator.get_no_of_samples() > 0:
        ...
        #     f.write('joint mae: {:.5f}, accel diff: {:.5f}, FGD: {:.3f}, feat_D: {:.3f}, PCK: {:.3f}, div: {:.3f} / {:.1f}s'.format(
        #         joint_mae.avg, accel.avg, frechet_dist, feat_dist, pck.avg, diversity.avg, elapsed_time))
        #     f.write('\n')
        print(
            'joint mae: {:.5f}, accel diff: {:.5f}, FGD: {:.3f}, feat_D: {:.3f}, PCK: {:.3f}, div: {:.3f} / {:.1f}s'.format(
                joint_mae.avg, accel.avg, frechet_dist, feat_dist, pck.avg, diversity.avg, elapsed_time))
...
def train_eval_FGD(eval_loader, graph, dataset, step):
    ...
    return evaluate_testset(dataset, eval_loader, graph, embed_space_evaluator, hparams_motion, step)
