...
class Generator(object):
    ...
    def prepare_text(self, text_data):
        nn, seqlen, n_feats = text_data.shape

        text_data = text_data.reshape((nn, seqlen * n_feats))
        text = torch.from_numpy(np.expand_dims(text_data, axis=-1))
        return text.to(self.data_device)
    ...
    def generate_sample(self, graph, eps_std=1.0, step=0, counter=0):
        ...
        text = batch["text"].cpu().numpy()
        text = torch.from_numpy(text).to(self.data_device)
        ...
        with torch.no_grad():
            for i in range(0, control_all.shape[1] - seqlen - n_lookahead, clip_length):
                ...
                seed = torch.from_numpy(sampled_all[:, :seqlen, :]).to(self.data_device)
                seed = seed.permute(0, 2, 1).float()
                control = torch.from_numpy(control_all).to(self.data_device)
                control = control.permute(0, 2, 1).float()

                # sample from Moglow
                sampled = graph(z=None, cond=cond, eps_std=eps_std, reverse=True, seed=seed, control=control, text=text)
    ...
    def style_transfer(self, graph, eps_std=1.0, style='AYzA2uyd9_s'):
        ...
        with torch.no_grad():
            ...
            text = batch['text'][idx].unsqueeze(0).to(self.data_device)
            seed0 = batch['seed'][idx].unsqueeze(0).to(self.data_device)
            control = batch['control'][idx].unsqueeze(0).to(self.data_device)
            z, nll = graph(x=x, cond=cond, seed=seed0, control=control, text=text) # z : [1, 27, 32]
        ...
        with torch.no_grad():
            for i in range(0, control_all.shape[1] - seqlen - n_lookahead, clip_length):
                ...
                # noise = torch.randn((nn, 1, n_feats)).to(self.data_device)

                seed = torch.from_numpy(sampled_all[:, :(i+seqlen), :]).to(self.data_device)
                seed = seed.permute(0, 2, 1).float()
                control = torch.from_numpy(control_all).to(self.data_device)
                control = control.permute(0, 2, 1).float()

                # sample from Moglow
                sampled = graph(z=z_style, cond=cond, eps_std=eps_std, reverse=True, seed=seed, control=control, text=text)
                sampled = sampled.cpu().numpy()[:, :, 0].reshape(sampled.shape[0], clip_length, -1)
    ...
    def generate_code(self, graph):
        ...
        for i_batch, batch in enumerate(progress):
            with torch.no_grad():
                ...
                seed=batch['seed'].to(self.data_device), control=batch['control'].to(self.data_device), text=batch['text'].to(self.data_device))
        ...
        # tsne_code = self.calc_pca(z_all)
        self.plot2D(tsne_code, style_all, outputfig)
        # self.cal_dis(z_all, style_all, outputfig)
        from sklearn.cluster import KMeans
        from sklearn import metrics
        kmean_model = KMeans(n_clusters=5).fit(tsne_code)
        labels = kmean_model.labels_
        print(metrics.silhouette_score(tsne_code, labels, metric="euclidean"))
        print(metrics.calinski_harabasz_score(tsne_code, labels))
