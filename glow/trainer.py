...
from scripts.test_FGD import train_eval_FGD
            ...
            for i_batch, batch in enumerate(progress):
                ...
                if self.global_step == 0:
                    self.graph(x=x[:self.batch_size // len(self.devices), ...],
                               ...
                               seed=seed, control=control, text=text)
                    # re-init LSTM hidden
                    if hasattr(self.graph, "module"):
                        self.graph.module.init_lstm_hidden()
                    else:
                ...
                # forward phase
                z, nll = self.graph(x=x, cond=cond, seed=seed, control=control, text=text) # z: [200, 27, 32], cond [200, 589, 32]
                                                    # control: [200, 42, 29]
                ...
                x_recon = self.graph(z=None, cond=cond, eps_std=1.0, reverse=True, seed=seed, control=control, text=text)
                ...
                if self.global_step % self.validation_log_gaps == 0:
                    ...
                    for ii, val_batch in enumerate(self.val_data_loader):
                        ...
                        with torch.no_grad():
                            ...
                            # val_noise = torch.randn_like(val_batch["x"].permute(0, 2, 1))
                            if self.is_trinity:
                                text = None
                            else:
                                text = val_batch["text"]
                            ...
                            seed=val_batch["seed"], control=val_batch["control"], text=text)
